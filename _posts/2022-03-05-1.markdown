...
Deploy changes to fleets of services using Two Phase Deployment: make two waves of changes, where the first wave ensures backwards compatibility with the current state and also forwards compatibility with the changes made in the second wave. That way, the second wave of changes can be rolled back and the system will not suffer any residual damage, as the first wave will have ensured forward compatibility.
